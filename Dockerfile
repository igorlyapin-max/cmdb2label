...
RUN has_customer_ca=0; \
    if find /usr/local/share/ca-certificates/cmdb2label-customer -type f \( -name '*.crt' -o -name '*.pem' \) ! -name '*.example' -print -quit | grep -q .; then has_customer_ca=1; fi; \
    ...
    if [ "$has_customer_ca" -eq 1 ]; then update-ca-certificates; fi
