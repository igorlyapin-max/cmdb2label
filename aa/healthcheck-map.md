...
| --- | --- | --- | --- | --- | --- | --- | --- |
| HC-001 | `H0` | `GET /health/live` | Docker healthcheck, LB, operator | `200` alive | Только процесс backend | service, `live`, safe build identity | Без данных CMDBuild |
| HC-002 | `H0` | `GET /health/ready` | LB, monitoring, operator | `200` ready, `503` not ready | Runtime config и доступность CMDBuild upstream | service, `ready`, status, safe build identity | Не раскрывает `CMDBUILD_ORIGIN` или raw error |
| HC-003 | `H0` | `GET /cmdbuild/custom-api/labels/health/live` | Same-origin proxy/monitoring | `200` alive | Только процесс backend | Как liveness | API-prefixed alias для proxy integration |
